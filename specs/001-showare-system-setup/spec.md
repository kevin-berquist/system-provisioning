...
- **FR-010**: When `ENFORCE_AUTH` is set to true, the application MUST perform a server-side GET request to the configured `AUTH_URL` for every incoming request (UI or API) before executing feature logic. The request MUST include both the `UserAccount` and `UserLoggedIn` cookie values received from the caller in the Cookie header. If and only if the response body contains the literal text `AUTH OK`, processing may continue; otherwise the application MUST terminate the request with HTTP 401 Unauthorized and no side effects.

- **FR-014**: When `DEBUG_AUTH_REQUEST` is set to true, the authentication enforcement function MUST return detailed debugging information as the HTTP response body instead of allowing normal request processing. The debug response MUST include: the complete request URL sent to `AUTH_URL`, all request headers sent, the full response content received from the auth service, all response headers returned by the auth service, and the final authentication status decision. This debug response facilitates troubleshooting authentication integration issues in development environments by providing complete visibility into the authentication flow.
